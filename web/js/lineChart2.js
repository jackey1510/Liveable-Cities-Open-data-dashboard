function nestedlineChart(divid, dropdownid,src, filter, id, xdomain, ydomain, zdomain, xlabel, ylabel, positivetrend, oneline,timeparse, filter2){
  ...
  var valueline = d3.line()
      ...
      .y(function(d) { return y(d[ydomain][0]); });
  ...
  d3.json(endpoint+filter, function(error, data) {
    ...
    var nestedData = d3.keys(data[0]).filter(function(key){
        return key !== "Area Codes" && key !=="Area Names" && key !=="id"  ;
      }).map(function(Year){
        return {
          Year: Year,
          Indicator_value: data.map(function(d){
            return d[Year];
          })
        }
      })

    var min = d3.min(nestedData,function(d){return d[ydomain][0]});
    var max = d3.max(nestedData,function(d){return d[ydomain][0]});
  ...
  d3.json(endpoint + src, function(error, data) {
    ...
    //max = d3.max(data,function(d){return d[ydomain]});
      ...
      x.domain(nestedData.map(function(d) { return d[xdomain]; }));
      //y.domain([0, d3.max(data, function(d) { return d[ydomain]; })]);
      y.domain([min,max]);
 
    //console.log(color(1));
    console.log(x.domain());
    console.log(x("2006-2008"));
